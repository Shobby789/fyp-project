...
import {View, TouchableOpacity, Text, Animated, Image, ViewBase} from 'react-native';
...
const TabIndicator = ({measureLayout, scrollX})=>{
  ...
  const tabIndicatorWidth = scrollX.interpolate({
    inputRange,
    outputRange: measureLayout.msp(measure => measure.x)
  })

  const translateX = scrollX.interpolate({
    inputRange,
    outputRange: measureLayout.msp(measure => measure.width)
  })
...
const Tabs = ({scrollX, onBottomTabPress}) => {
      ...
      React.useEffect(() => {
          ...
          bottom_tabs.forEach(bottom_tabs => {
            bottom_tabs?.ref?.current?.measureLayout(
...
const MainLayout = () => {
  ...
  const onBottomTabPress = React.userCallback(bottomTabIndex => {
